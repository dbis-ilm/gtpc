...
std::vector<const char*> DataSource::tpchAuxiliaries={
	"do", "may", "might", "shall",
...
std::string DataSource::tpchNounPhrase(){
	if(randomTrue(1/4))
		return std::string(tpchNouns[randomUniformInt(0,tpchNouns.size()-1)]);
	else if(randomTrue(1/4))
		return std::string(tpchAdjectives[randomUniformInt(0,tpchAdjectives.size()-1)])  + " " +
		...
		return std::string(tpchAdverbs[randomUniformInt(0,tpchAdverbs.size()-1)])  + " " +
		tpchAdjectives[randomUniformInt(0,tpchAdjectives.size()-1)]  + " " + tpchNouns[randomUniformInt(0,tpchNouns.size()-1)];
}

std::string DataSource::tpchVerbPhrase(){
	if(randomTrue(1/4))
		return std::string(tpchVerbs[randomUniformInt(0,tpchVerbs.size()-1)]);
	else if(randomTrue(1/4))
		return std::string(tpchAuxiliaries[randomUniformInt(0,tpchAuxiliaries.size()-1)])  + " " +
		tpchVerbs[randomUniformInt(0,tpchVerbs.size()-1)];
	else if(randomTrue(1/4))
		return std::string(tpchVerbs[randomUniformInt(0,tpchVerbs.size()-1)]) + " " + tpchAdverbs[randomUniformInt(0,tpchAdverbs.size()-1)];
	else
		return std::string(tpchAuxiliaries[randomUniformInt(0,tpchAuxiliaries.size()-1)])  + " " +
...
std::string DataSource::tpchPrepositionalPhrase(){
	return std::string(tpchPrepositions[randomUniformInt(0,tpchPrepositions.size()-1)]) + " the " + tpchNounPhrase();
}
...
void DataSource::addAlphanumeric62(int length, std::ofstream& stream, bool delimiter){
	std::string s = "";
	int rand;
	for(int i=0; i<length; i++){
		rand=0;
...
void DataSource::addAlphanumeric64(int length, std::ofstream& stream, bool delimiter){
	std::string s = "";
	int rand;
	for(int i=0; i<length; i++){
		rand=0;
...
void DataSource::addAlphanumeric64(int minLength, int maxLength, std::ofstream& stream, bool delimiter){
	addAlphanumeric64(randomUniformInt(minLength,maxLength),stream,delimiter);
}

void DataSource::addAlphanumeric64Original(int minLength, int maxLength, std::ofstream& stream, bool delimiter){
	int rLength=0;
...
void DataSource::addTextString(int minLength, int maxLength, std::ofstream& stream, bool delimiter){
	stream << tpchText(randomUniformInt(minLength,maxLength));
	// if(delimiter)
	// 	stream << Config::getCsvDelim();
}

void DataSource::addTextStringCustomer(int minLength, int maxLength, const char* action, std::ofstream& stream, bool delimiter){
	int rLength=0;
	randomUniformInt(minLength,maxLength,rLength);
	int l1=0;
...
void DataSource::addInt(int minValue, int maxValue, std::ofstream& stream, bool delimiter){
	stream << randomUniformInt(minValue, maxValue);
	// if(delimiter)
	// 	stream << Config::getCsvDelim();
}

void DataSource::addDouble(double minValue, double maxValue, int decimals, std::ofstream& stream, bool delimiter){
	double d = 0;
